...
class HomeTjDataData extends BaseData
{
	...
	public function getDp($pageCore,$cateId=false)
	{
		...
		if($cateId)
		{
			$sql = 'select * from home_tj_data where cid = '.$cateId.' and time_end > '.time().' and state = '.HomeTjDataDataModel::STATE_HAVE.' order by ctime desc,sort desc limit '.$start.','.$end;
			$countSql = 'select count(*) from home_tj_data where cid = '.$cateId.' and time_end > '.time().' and state = '.HomeTjDataDataModel::STATE_HAVE;
		}else{
			$sql = 'select t1.* from home_tj_data as t1 inner join home_tj_class as t2 on t1.cid = t2.id where (t2.id = 2 or t2.pid = 2) and t1.time_end > '.time().' and  t1.state = '.HomeTjDataDataModel::STATE_HAVE.' order by t1.ctime desc, t1.sort desc limit '.$start.','.$end;
