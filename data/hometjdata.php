...
class HomeTjDataData extends BaseData
{
	...
	public function getDp($pageCore,$cateId=false)
	{
		...
		if($cateId)
		{
			$sql = 'select * from home_tj_data where cid = '.$cateId.' and t1.time_end < '.time().' and state = '.HomeTjDataDataModel::STATE_HAVE.' order by ctime desc,sort desc,id desc limit '.$start.','.$end;
			$countSql = 'select count(*) from home_tj_data where cid = '.$cateId.' and t1.time_end < '.time().' and state = '.HomeTjDataDataModel::STATE_HAVE;
		}else{
